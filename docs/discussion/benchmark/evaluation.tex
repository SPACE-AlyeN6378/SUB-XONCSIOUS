...
the callback execution times for each components are measured using the RAII timer (\Cref{app:exec_time}, Listing \ref{lst:raii-timer}) outside 
the audio engine. A buffer size of 512 floats, and a sample rate $f_s = $ \SI{44100}{\hertz} is used. 

The buffer latency is,
$$\frac{512}{44100} \approx 11.61\, \text{ms}$$
...
\input{discussion/benchmark/tables/components.tex}

It is observed from \Cref{tab:audio_component_benchmark} that reverb has the worst callback times of all, which is expected, because of the heavy
...
\input{discussion/benchmark/figures/daw_mockup.tex}
...
Thread 1 takes the longest amount of time to run compared to other threads, so the path from Thread 1 makes it
a critical path. So the other utilities, including the reverb mixer and the master mixer has to wait for at 
least \SI{228.42}{\micro\second}. Then, the reverb has to process the audio for about \SI{810.875}{\micro\second}. 
Therefore, the overall mean execution time down that critical path would be,

$$
\bar{t} = 228.42 + 810.875 = 1039.295\,\mu s = 1.039\, \text{ms}\, (< 11.6 \text{ms})
$$

Which means that the audio engine will work smoothly without any interruptions or artifacts. Additionally,
...
the buffer deadline. And finally, when the program is run on a single thread, the total mean execution time
would be \SI{1186.609}{\micro\second}.

Therefore, these estimations provide evidence that the DAW audio engine has the potential to survive that amount of workload. But the scalability
can further be extended by adding more tracks to perform the actual stress test, including getting everything to work together inside the audio engine 
