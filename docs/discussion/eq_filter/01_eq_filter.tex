...
but to be real-time friendly during the processing of each sample. The first digital 
filter implemented in the audio engine is the Robert Bristow-Johnson (RBJ) Biquad Filter, which is one of these second-order 
Infinite Impulse Response (IIR) filter \cite{smith_biquad}. 

The filter design is based on the biquadratic filter \cite{vanWaterschootT.2007APPT} 
using the Robert Bristow-Johnson (RBJ) method \cite{cookbook} to calculate
...
are automated. Please check \Cref{app:biquad_equations} for the list of equations and its corresponding code implementation.
...
\input{discussion/eq_filter/figures/rbj_bode_plot.tex}
...
Biquad filter is chosen for creating Parametric equalizers (\cref{prompt:parametric_eq}) 
\cite{openai_chatgpt,vanWaterschootT.2007APPT}.